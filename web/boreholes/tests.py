...
def createTestObject(user, **kwargs):
    ...
    request = HttpRequest()
    request.method = 'POST'
    request.read = lambda: json.dumps({k: kwargs[k] for k in kwargs.keys(
    ) if k in ('name', 'latitude', 'longitude', 'description')}).encode()
    request.user = user
...
class BoreholesViewTestCase(django.test.TestCase):
    ...
    def test01new(self):
        ...
        for i in range(self.test_num):
            ...
            # if assertions were passed, we can assume that views.new() (used
            # by self.insertNumObjects) works correctly
        ...
        b = Borehole.objects.filter(
            name=test_name, latitude=str(
                self.test_lat), longitude=str(
                self.test_lon), description=self.test_desc)
        ...
        # only one object should be returned as above insertion should fail (no
        # duplicates)
        self.assertEqual(len(b), 1)
    ...
    def test02get(self):
        ...
        # if previous test passed, we can use insertNumObjects and rely that it
        # will correctly create objects in DB
        self.insertNumObjects()
        ...
        request.method = 'GET'
        request.user = self.user
        bs = json.loads(views.boreholes(
            request).content.decode('utf-8'))['boreholes']
    ...
    def test04modify(self):
        ...
        self.assertRaises(
            DataError, self.modifyAndGetById, self.test_num, {
                'name': self.test_long_name})
