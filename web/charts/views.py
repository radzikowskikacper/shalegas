...
def charts(request, borehole_id):
    if request.method == 'GET':
        ...
        ret = list()
        if not 'unit_pair' in request.GET:
            for v in data:
                temp = next((item for item in ret if item['name'] == dicts[meanings[v.meaning_id].name]), None)
                to_add = v.to_dict()
                for i in [0, 2, 3]:
                    to_add[i] /= 100.0
                if not temp:
                    ret.append({'name' : dicts[meanings[v.meaning_id].name], 'unit' : meanings[v.meaning_id].unit, 'data' : [[to_add]]})
                else:
