...
def tables(request, borehole_id):
    if request.method == 'GET':
        with transaction.atomic():
            ...
            rmeasurements = RealMeasurement.objects.filter(
                **params).order_by('depth_from', 'meaning')
            dmeasurements = DictionaryMeasurement.objects.filter(
                **params).exclude(
                meaning_id__in=MeaningDict.objects.filter(
                    section='Stratygrafia')).order_by(
                'depth_from', 'meaning')
            if filter:
            ...
            ret['header'] += ([{'name': meanings[d.meaning_id].name,
                                'unit': meanings[d.meaning_id].unit} for d in temp])
