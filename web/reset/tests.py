...
class ResetViewsTestCase(django.test.TestCase):
    ...
    def test01prepareDumpName(self):
        self.assertEqual(
            views.prepareDumpName(), "%s/SweetSpot_%s.sql" %
            (DUMPS_DIRNAME, datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")))

        f = open('reset/dumps/SweetSpot_%s.sql' %
                 datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S"), 'w+')
        f.close()

        self.assertEqual(
            views.prepareDumpName(), "%s/SweetSpot_%s (1).sql" %
            (DUMPS_DIRNAME, datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")))

        for i in range(1, 6):
            f = open(
                'reset/dumps/SweetSpot_%s (%d).sql' %
                (datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S"), i), 'w+')
            f.close()

        self.assertEqual(
            views.prepareDumpName(), "%s/SweetSpot_%s (6).sql" %
            (DUMPS_DIRNAME, datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")))
    ...
    def test02getDumpFiles(self):
        ...
        self.assertListEqual(sorted(views.getDumpFiles()), sorted(['SweetSpot_%s (%d)' % (
            fnames[i], i) for i in fnames] + ['SweetSpot (%d)' % i for i in range(1, 6)]))
    ...
    def test05getDumpFiles(self):
        ...
        self.assertEqual(
            len(json.loads(views.reset(self.request).content.decode('utf-8'))), 10)
