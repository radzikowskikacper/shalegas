...
def getDumpFiles():
    return [os.path.splitext(f)[0] for f in os.listdir(DUMPS_DIRNAME) if os.path.isfile(os.path.join(DUMPS_DIRNAME,f))]

def prepareDumpName():
    fname = fname_format = "SweetSpot_%s" % datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    i = 1
...
def reset(request):
    if request.method == 'GET':
        ...
        if fname and os.path.isfile('%s/%s.sql' % (DUMPS_DIRNAME, fname)):
            logger.info("User %s started restoring database %s.sql ..." % (request.user.username, fname))
            
            os.system("PGPASSWORD='%s' pg_restore -U %s -d %s --clean < %s/%s.sql" % (version.models.getDBPassword(), 
        ...
        if len(getDumpFiles()) < settings.MAX_DUMP_FILES:
            os.system("PGPASSWORD='%s' pg_dump -U %s -Fc %s > %s" % (version.models.getDBPassword(), version.models.getDBUser(),
