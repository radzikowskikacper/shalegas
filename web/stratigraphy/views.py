        ...
        with transaction.atomic():
            ...
            for d in MeaningDict.objects.filter(section='Stratygrafia'):
                if str(d.id) in params:
                    ...
                    DictionaryMeasurement.objects.create(borehole=Borehole.objects.get(id=borehole_id),
            ...
            logger.info(
                "User %s added stratigraphy entry" %
                request.user.username)
            return getStratigraphy(
                **dict(params['query'], **{'borehole_id': borehole_id}))
