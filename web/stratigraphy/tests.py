...
class MeasurementsModelTestCase(django.test.TestCase):
    ...
    def test01getStratigrahpy(self):
        ...
        for index, did in enumerate(stratigraphy_list):
            for i in range(20 + index * 2, 20 + self.tests_num + index * 2):
                for j in range(2):
                    self.assertTrue({'thill': self.test_depth + j * 100,
                                     'ceil': self.test_depth + (j + i) * 100,
    ...
    def test02addStratigraphy(self):
        ...
        errortab = [KeyError for i in range(
            5)] + [MeaningDictValue.DoesNotExist]
        ...
        self.request.read = lambda: json.dumps(
            {
                603: '20',
                604: '22',
                'depth_from': '%d' % (self.test_depth / 100),
                'depth_to': '%d' % (self.test_depth / 100 + 20),
                'geophysical_depth': '%d' % (self.test_geophysical_depth / 100),
                'query': {
                    'type': 'STRAT'}}).encode()
        res = json.loads(
            views.stratigraphy(
        ...
        self.assertListEqual(res,
                             [{'thill': self.test_depth,
                               'ceil': self.test_depth + 2000,
        ...
        self.request.read = lambda: json.dumps(
            {
                603: '20',
                'depth_from': '%d' % (self.test_depth / 100),
                'depth_to': '%d' % (self.test_depth / 100 + 21),
                'geophysical_depth': '%d' % (self.test_geophysical_depth / 100),
                'query': {
                    'type': 'STRAT'}}).encode()
        self.assertEqual(len(json.loads(views.stratigraphy(
            self.request, 1).content.decode('utf-8'))), 2)
        ...
        self.request.read = lambda: json.dumps(
            {
                ...
                'query': {
                    'type': 'STRAT'}}).encode()
        self.assertEqual(len(json.loads(views.stratigraphy(
            self.request, 1).content.decode('utf-8'))), 3)
