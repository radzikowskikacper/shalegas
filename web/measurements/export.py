...
def writeTopHeader(writer, request, borehole):
    ...
    to_add = [
        six.text_type(
            borehole.name), user, six.text_type(
...
def getRows(
    ...
    params = {
        'meaning_id__in': chosenMeaningsId,
        'depth_from__lte': depth_to,
        'depth_to__gte': depth_from,
        'borehole': borehole}
    measurements = RealMeasurement.objects.filter(
        **params).order_by('depth_from')
    dictionaries = DictionaryMeasurement.objects.filter(
        **params).order_by('depth_from')
    measurements = sorted(
        list(measurements) +
        list(dictionaries),
        key=lambda s: s.depth_from)
    ret = []
    if (len(measurements) > 0):
        ...
        while True:
            ...
            row = ExportRow(
                row_counter,
                measurements[i].depth_from,
                measurements[i].geophysical_depth,
                len(meaningColDict))
            while True:
                if (i == len(measurements)
                        or measurements[i].depth_from != row.depth_from):
                    break
                row.insertValue(meaningColDict[measurements[i].meaning_id], measurements[i].value if hasattr(
                    measurements[i], 'value') else dicts.get(measurements[i].dictionary.value, measurements[i].dictionary.value))
                i += 1
