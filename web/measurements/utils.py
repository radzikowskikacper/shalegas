...
def prepareFilter(**kwargs):
    ...
    bh = Borehole.objects.get(id=(kwargs['borehole_id'][0] if isinstance(
        kwargs['borehole_id'], list) else kwargs['borehole_id'])) if mtype != 'ALL_BHS' else None

    strattable = None
    if 'strat' in kwargs:
        strats = DictionaryMeasurement.objects.filter(
            meaning_id__in=MeaningDict.objects.filter(
                section='Stratygrafia'), dictionary__in=kwargs['strat'])
        if bh:
            strats = strats.filter(borehole_id=bh)
        strattable = intervals_calculator(
            [(m.depth_from, m.depth_to) for m in strats.order_by('depth_from', 'depth_to')])

    if mtype != 'ALL_BHS':
        params['borehole_id'] = bh

        if 'start_depth' in kwargs:
            params['depth_to__gte'] = int(
                float(
                    kwargs['start_depth'][0] if isinstance(
                        kwargs['start_depth'],
                        list) else kwargs['start_depth'])) * 100
        if 'stop_depth' in kwargs:
            params['depth_from__lte'] = int(
                float(
                    kwargs['stop_depth'][0] if isinstance(
                        kwargs['stop_depth'],
                        list) else kwargs['stop_depth'])) * 100

        if strattable:
            filter_intersection = False
            measurement_size = int(
                settings.MEASUREMENT_IMAGE_HEIGHT_CM) if mtype == 'PICT' else 1
            for s in strattable:
                if not(
                        params['depth_to__gte'] -
                        s[1] > measurement_size or s[0] -
                        params['depth_from__lte'] > measurement_size):
    ...
    if isinstance(strattable, list):
        if len(strattable):
            filter = Q(
                depth_to__gte=strattable[0][0]) & Q(
                depth_from__lte=strattable[0][1])
            for s in strattable[1:]:
                filter |= Q(depth_to__gte=s[0]) & Q(depth_from__lte=s[1])
        else:
            filter = Q(
                depth_to__gte=settings.MAX_BOREHOLE_HEIGHT) & Q(
                depth_from__lte=-1)
    ...
    params['meaning__in'] = meanings = dict(
        (i.id, i) for i in list(meanings.order_by('id')))
...
def getMeasurements(**kwargs):
    ...
    if mtype == 'NDICT':
        ...
        ret = [v.to_dict() + [meanings[v.meaning_id].name,
                              meanings[v.meaning_id].unit] for v in data]
    elif mtype == 'DICT':
        data = DictionaryMeasurement.objects.filter(
            **params).order_by('meaning', 'depth_from')
        if filter:
            data = data.filter(filter)
        ...
        if filter:
            data = data.filter(filter)
        ret = [{"id": img[0], "depth_from": img[1], "depth_to": img[2],
                "geophysical_depth": img[3], "meaning": img[4]} for img in data]
