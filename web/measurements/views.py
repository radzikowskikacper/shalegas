...
def measurements(request, id=None):
    ...
    if request.method == 'GET':
        ...
        params = json.loads(request.read().decode(
            'utf-8')) if not len(request.FILES) else request.POST
        ...
        with transaction.atomic():
            ...
            if 'query' not in params and params['type'] == 'PICT':
                depth_to = drilldepth + \
                    int(settings.MEASUREMENT_IMAGE_HEIGHT_CM)
                meaning = MeaningImage.objects.get(id=params['meaning'])
                add_image(
                    request.FILES["image_path"].read(),
                    borehole=borehole,
                    depth_from=drilldepth,
                    depth_to=depth_to,
                    geophysical_depth=geodepth,
                    meaning=meaning)
                logger.info(
                    "User %s added new photo measurement" %
                ...
                if not RealMeasurement.objects.filter(
                    borehole=borehole,
                    depth_from=drilldepth,
                    meaning=MeaningValue.objects.get(
                        id=params['meaning'])).exists():
                    RealMeasurement.objects.create(
                        ...
                        meaning=MeaningValue.objects.get(
                            id=params['meaning']))
                    logger.info(
                        "User %s added new real measurement" %
                        request.user.username)

                    return getMeasurements(
                        **dict(params['query'], **{'borehole_id': id}))
                else:
                ...
                if not DictionaryMeasurement.objects.filter(
                        borehole=borehole, depth_from=drilldepth, meaning=meaning).exists():
                    DictionaryMeasurement.objects.create(
                        ...
                        dictionary=MeaningDictValue.objects.get(
                            id=params['dictionary'],
                            dict_id=meaning))
                    logger.info(
                        "User %s added new dictionary measurement" %
                        request.user.username)
        ...
        with transaction.atomic():
            ...
            if params['query']['type'] == 'NDICT':
                ...
                RealMeasurement.objects.filter(
                    borehole=borehole, meaning__in=MeaningValue.objects.filter(
                        id__in=chosenMeanings)).delete()
                DictionaryMeasurement.objects.filter(
                    borehole=borehole, meaning__in=MeaningDict.objects.filter(
                        id__in=chosenMeanings)).delete()
                Img.objects.filter(
                    borehole=borehole,
                    meaning__in=chosenMeanings).delete()
                return HttpResponse()
            ...
            return getMeasurements(
                **dict(params['query'], **{'borehole_id': params['borehole_id']}))
    ...
    with transaction.atomic():
        ...
        for row in reader:
            ...
            if not dict_measurement:
                ...
                if not DictionaryMeasurement.objects.filter(
                        borehole=borehole, depth_from=depth_from, meaning=meaning).exists():
                    if sys.version_info[:1][0] < 3:
        ...
        logger.info(
            'User %s added %d rows from csv file: ' %
...
def intervals(
    ...
    paramsvals = {
        'depth_from__lte': stop_depth,
        'depth_to__gte': start_depth,
        'borehole': borehole}
    paramsdicts = {
        'depth_from__lte': stop_depth,
        'depth_to__gte': start_depth,
    ...
    if 'filter' in request.GET:
        ...
        paramspicts['meaning__in'] = MeaningImage.objects.filter(
            id__in=filters)
    else:
        paramsdicts['meaning__in'] = MeaningDict.objects.all()

    paramsdicts['meaning__in'] = paramsdicts['meaning__in'].exclude(
        section='Stratygrafia')

    valssects = [(m.depth_from, m.depth_to)
                 for m in RealMeasurement.objects.filter(**paramsvals)]
    dictsects = [(m.depth_from, m.depth_to)
                 for m in DictionaryMeasurement.objects.filter(**paramsdicts)]
    pictsects = [(m.depth_from, m.depth_to) for m in Img.objects.filter(
        **paramspicts).exclude(meaning__isnull=True)]
    sections = intervals_calculator(
        sorted(
            valssects +
            dictsects +
            pictsects,
    ...
    for s in sections:
        height = int((width * (s[0] - start_depth) +
                      (width * (s[1] - start_depth))) / 2)
        draw.line([(0, height), (outimg_width, height)], fill='#ff0000',
                  width=int(width * (s[1] - start_depth) - height) * 2)
