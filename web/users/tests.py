...
class UsersViewTestCase(django.test.TestCase):
    ...
    def createTestUsers(self):
        ...
        for i in range(self.test_users_num):
            user = User.objects.create_user(self.test_username + str(i), '', self.test_password + str(i))
    ...
    def test02login(self):
        ...
        #incorrect login
        self.assertRaises(views.BadUsername, self.login, 'a', self.test_password, AnonymousUser())
        self.assertFalse(self.request.user.is_authenticated())

        self.assertRaises(views.BadUsername, self.login, self.test_username, 'b')
        self.assertFalse(self.request.user.is_authenticated())
        ...
        self.assertEqual(json.loads(resp.content.decode('utf-8'))['username'], self.test_username)
    ...
    def test03getUsers(self):
        ...
        #users' number should be $self.test_users_num + 1 (sweetspot, sweetspot)
        ...
        user = json.loads(views.users(self.request, self.test_users_num + 1).content.decode('utf-8'))
        #test his properties
        self.assertEqual(user['username'], self.test_username + str(self.test_users_num - 1))
        self.assertEqual(user['first_name'], self.test_first_name + str(self.test_users_num - 1))
        self.assertEqual(user['last_name'], self.test_last_name + str(self.test_users_num - 1))
        
        self.assertDictEqual(json.loads(views.users(self.request, 1).content.decode('utf-8')), 
                             {'username' : 'sweetspot', 'id' : six.text_type(1), 'last_name' : '', 'first_name' : '', 
                              'readwrite' : True})

        #try to get nonexisting user
        self.assertRaises(User.DoesNotExist, views.users, self.request, self.test_users_num + 2)
    ...
    def test04passwordChange(self):
        ...
        #as there is no field 'old_password', bad self.request should be returned
        self.request.read = lambda: json.dumps(params).encode()
        self.assertRaises(KeyError, views.users, self.request, 2)

        params['old_password'] = self.test_password + '_different'
        self.assertRaises(views.IncorrectPassword, views.users, self.request, 2)
        ...
        self.assertTrue(check_password(self.test_new_password, User.objects.get(id=2).password))
    ...
    def test05personalDataChange(self):
        ...
        self.request.method = 'PUT'
        self.request.session = self.engine.SessionStore(self.session_key)
        params = {'first_name' : self.test_first_name, 'last_name' : self.test_long_name,
        ...
        self.assertFalse(self.request.user.groups.filter(name='editors').exists())
        
        params['readwrite'] = True
        views.users(self.request, 1)
        self.assertTrue(self.request.user.groups.filter(name='editors').exists())
    ...
    def test05userDelete(self):
        ...
        #total amount of users should still be self.test_users_num + 1 (sweetspot, sweetspot)
    ...
    def test06addUser(self):
        ...
        self.request.read = lambda : json.dumps({'login' : 'test_login', 'password' : 'test_password', 'fname' : 'test_fname', 
                                                 'lname' : 'test_lname', 'password_test' : 'test_password1'}).encode()
        self.assertRaises(PasswordMismatch, views.users, self.request)

        self.request.read = lambda : json.dumps({'login' : 'test_login', 'password' : 'test_password', 'fname' : 'test_fname', 
                                                 'lname' : 'test_lname', 'password_test' : 'test_password'}).encode()
