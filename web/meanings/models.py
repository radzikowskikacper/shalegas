    ...
    if 'mid' in kwargs:
        ...
        if 'filter' in kwargs:
            if kwargs['filter'][0] == 'DICT':
                ret = [
                    {
                        'name': s.name, 'meanings': [
                            createElement(u) for u in MeaningDict.objects.filter(
                                section=s) .order_by('name')]} for s in MeaningSection.objects.order_by('name').exclude(
                        name='Stratygrafia')]
                ...
                ret = [
                    {
                        ...
                        'meanings': [
                            createElement(u) for u in MeaningValue.objects.filter(
                                ...
                                unit__in=[
                                    'DICT',
                                    'PICT']).order_by('name')]} for s in MeaningSection.objects.order_by('name')]
            elif kwargs['filter'][0] == 'PICT':
                ret = [{'name': s.name, 'meanings': [createElement(u) for u in MeaningImage.objects.filter(
                    section=s).order_by('name')]} for s in MeaningSection.objects.order_by('name')]
            elif kwargs['filter'][0] == 'STRAT':
                ret = [
                    createElement(u) for u in MeaningDict.objects.filter(
                        section='Stratygrafia').order_by('id')]
        else:
            ret = [{'name': s.name, 'meanings': [createElement(u) for u in MeaningValue.objects.filter(
                section=s) .order_by('name')]} for s in MeaningSection.objects.order_by('name')]
