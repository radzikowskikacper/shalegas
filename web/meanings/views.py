        ...
        if request.method == 'GET':
            ...
            with transaction.atomic():
                if not MeaningSection.objects.filter(name = name).exists():
                    ...
                    logger.info("User %s added new section" % request.user.username)
...
def meanings(request, meaning_id = None):
    if not meaning_id:
        if request.method == 'GET':
            ...
            with transaction.atomic():
                if not MeaningValue.objects.filter(name = name, section = sect).exists():
                    if params['type'] == 'normal':
                        MeaningValue.objects.create(name = name, unit = params['unit'], 
                                                    section = MeaningSection.objects.get(name = sect))
                    elif params['type'] == 'pict':
                            MeaningImage.objects.create(name = name, unit = 'PICT', 
                                                    section = MeaningSection.objects.get(name = sect))
                    elif params['type'] == 'dict':
                        d = MeaningDict.objects.create(name = name, section = MeaningSection.objects.get(name=sect),
                                                       unit = 'DICT')
                        
                        for val in params['dictvals']:
                            MeaningDictValue.objects.create(value = val, dict_id = d)
                    else:
                        raise KeyError()
                    logger.info("User %s added new meaning" % request.user.username)
            ...
            with transaction.atomic():
                ...
                if params['type'] == 'dict':    
                    if meaning.unit != 'DICT':
                        ...
                        name = meaning.name
                        meaning.delete()
                        meaning = MeaningDict.objects.create(name = name, section = sect, unit = 'DICT')

                    MeaningDictValue.objects.filter(value__in =
                        set([v[0] for v in MeaningDictValue.objects.filter(dict_id = 
                                                            meaning).values_list('value')]).difference(set(params['dictvals']))).delete()                        
    
                    for val in set(params['dictvals']).difference(set([v[0] for v in MeaningDictValue.objects.filter(dict_id = 
                        meaning).values_list('value')])):
                        MeaningDictValue.objects.create(dict_id = meaning, value = val)
                    ...
                    if params['type'] == 'pict':
                        if meaning.unit != 'PICT':
                            meaning.delete()
                            meaning = MeaningImage.objects.create(name = name, section = sect, unit="PICT")
                            
                    elif params['type'] == 'normal':
                        if meaning.unit in ['PICT', 'DICT']:
                            meaning.delete()
                            meaning = MeaningValue.objects.create(name = name, section = sect)
                ...
                if 'section'  in params:
                    meaning.section = MeaningSection.objects.get(name = params['section'])
