...
def image(request, measurement_id=None):
    ...
    if request.method == 'POST':  # upload file here
        ...
        depth_to = float(
            request.POST['depth_to']) * 100 if (
            'depth_to' in request.POST and int(
                request.POST['depth_to']) * 100 > depth_from) else depth_from + float(
    ...
    if request.method == 'PUT':
        ...
        borehole = Borehole.objects.get(id=json.loads(
            request.read().decode('utf-8'))["borehole_id"])
        ...
        logger.info(
            "User %s started photo regeneration" %
    ...
    imgs = models.Image.objects.filter(
        meaning=None, borehole=Borehole.objects.get(
            id=request.GET["borehole_id"])).order_by('depth_from')
    imgs = imgs.values_list(
        'id',
        'depth_from',
        'depth_to',
        'geophysical_depth')
    ...
    if "stop_depth" in request.GET:
        imgs = imgs.filter(
            depth_from__lte=int(
                request.GET["stop_depth"]) * 100)

    return _JsonResponse([{"id": img[0],
                           "depth_from": img[1],
                           "depth_to": img[2],
                           "geophysical_depth": img[3]} for img in imgs])
...
def borehole_image(
    ...
    if depthRange != 1:
        ...
        imgs = imgs.extra(
            where=[
                ...
                (imgheight * image_height_cm)]).order_by('depth_from')
    else:
        imgs = models.Image.objects.filter(borehole=borehole_id)
        imgs = imgs.filter(Q(depth_from__range=(start_depth, stop_depth)) | Q(depth_to__range=(
            start_depth, stop_depth)))  # depth_from=start_depth, depth_to=stop_depth)
        imgs = imgs.filter(depth_to=F('depth_from') +
    ...
    if depthRange != 1 or outimg_width != 0 and outimg_height != 0:
        outimg = Image.new(
            "RGBA", (int(
                settings.MEASUREMENT_IMAGE_WIDTH_PX), int(
                imagePosHeight(
                    stop_depth - start_depth) / imgheight)))
        for img in imgs:
            if img.depth_to != img.depth_from + image_height_cm * imgheight:
                logger.warning(
                    'image: bad height in [cm] for id={img_id}, from={depth_from}, to={depth_to}, height={height}'.format(
                        img_id=img.id,
            ...
            outimg.paste(
                imgdata, (0, int(
                    imagePosHeight(
                        img.depth_from - start_depth) / imgheight)))
        outimg = outimg.resize(
            (outimg_width, outimg_height), resample=Image.ANTIALIAS)
    else:
