...
class ImageViewTestCase(django.test.TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.bh1 = Borehole.objects.create(id=1, name="bh1", latitude=0, longitude=0, description="")
        for i in range(self.test_bh1_num):
            self.img[i] = Image.objects.create(borehole=self.bh1, depth_from=i*100, depth_to=(i+1)*100, geophysical_depth=0, imagedata=self.imgw.getvalue())

        self.bh2 = Borehole.objects.create(id=2, name="bh2", latitude=10, longitude=10, description="")
        for i in range(self.test_num - self.test_bh1_num):
            Image.objects.create(borehole=self.bh2, depth_from=i*100, depth_to=i*100+100, geophysical_depth=0, imagedata=self.imgb.getvalue())

        MeaningImage.objects.create(name = 'meaning', id = 10, section = MeaningSection.objects.create(name = 'section'),
                                    unit = 'unit')
    ...
    def setUpClass(self):
        self.imgw = BytesIO()
        Im.new("RGB", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX), "white").save(self.imgw, format="JPEG")

        self.imgsize = BytesIO()
        Im.new("RGB", (int(settings.MEASUREMENT_IMAGE_WIDTH_PX / 2), int(settings.MEASUREMENT_IMAGE_HEIGHT_PX / 2)), "white").save(self.imgsize, format="JPEG")

        self.imgb = BytesIO()
        Im.new("RGB", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX), "black").save(self.imgb, format="JPEG")
        
        self.imgbig = BytesIO()
        Im.new("RGB", (settings.MEASUREMENT_IMAGE_WIDTH_PX + 100, settings.MEASUREMENT_IMAGE_HEIGHT_PX + 100), "white").save(self.imgbig, format="JPEG")
    ...
    def test02GetBh(self):
        ...
        res = views.image(self.req)
        self.assertListEqual(json.loads(res.content.decode('utf-8')),[{'id' : 1, 'depth_from' : 0, 'depth_to' : 100, 'geophysical_depth' : 0},
                                                      {'id' : 2, 'depth_from' : 100, 'depth_to' : 200, 'geophysical_depth' : 0},
                                                      {'id' : 3, 'depth_from' : 200, 'depth_to' : 300, 'geophysical_depth' : 0}])
    ...
    def test06UploadImage1(self):
        ...
        self.req.POST = QueryDict('borehole_id=%d&depth_from=900&geophysical_depth=900' % self.bh1.id)
        ...
        #without 'dept_from'
        self.req.method = 'POST'
        self.req.POST = QueryDict('borehole_id=%d&geophysical_depth=1000' % self.bh1.id)
        self.req.FILES = dict(image_path=BytesIO(self.imgsize.getvalue()))
        res = views.image(self.req)
        self.assertEqual(res.status_code, 415)
        #without 'geophysical_depth'
        
        self.req.POST=QueryDict('borehole_id=%d&depth_from=1100' % self.bh1.id)
        self.req.FILES = dict(image_path=BytesIO(self.imgw.getvalue()))
        res = views.image(self.req)
        self.assertEqual(res.status_code, 200)
        ...
        self.req.method = 'POST'
        self.req.POST = QueryDict('borehole_id=%d&depth_from=9000&geophysical_depth=900&meaning=10' 
                                  % self.bh1.id)
        self.req.FILES = dict(image_path=BytesIO(self.imgw.getvalue()))
        res = views.image(self.req)
        self.assertEqual(res.status_code, 200)
    ...
    def test07imagePositionHeight(self):
        ...
        self.assertEqual(views.imagePosHeight(image_height_cm), image_height_px )
        self.assertEqual(views.imagePosHeight(2*image_height_cm), 2*image_height_px )
        self.assertEqual(views.imagePosHeight(3*image_height_cm), 3*image_height_px )
    
    def test08UploadArchive(self):
        self.req.method = 'POST'
        self.assertRaises(Borehole.DoesNotExist, views.upload_archive, self.req, -99)
    ...
    def test10BoreholeImage(self):
        ...
        self.req.method = 'GET'
        self.assertRaises(Exception, views.borehole_image, self.req, self.bh1.id, 3, 0, settings.MEASUREMENT_IMAGE_WIDTH_PX, 3 * settings.MEASUREMENT_IMAGE_HEIGHT_PX)
        
        # image is too big
        
        img_size = (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX)
        Image.objects.create(borehole = self.bh1, depth_from = 0, depth_to = 500000, geophysical_depth = 0, imagedata = self.imgw.getvalue())
        res = views.borehole_image(self.req, self.bh1.id, 0, 5000, settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX)
        self.assertEqual(res.status_code, 200)
        img = Im.open(BytesIO(res.content))
        self.assertEqual(img.size, img_size)
        colors = img.getcolors()
        self.assertEqual(len(colors), 1)
        self.assertEqual(colors, [(img_size[0]*img_size[1], (255, 255, 255))])
        
        # get image from where there are measurements
        img_size = (settings.MEASUREMENT_IMAGE_WIDTH_PX, 3 * settings.MEASUREMENT_IMAGE_HEIGHT_PX)
        res = views.borehole_image(self.req, self.bh1.id, 0, 3, img_size[0], img_size[1])
        self.assertEqual(res.status_code, 200)
        ...
        img_size = (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX)
        res = views.borehole_image(self.req, self.bh1.id, 5, 6, img_size[0], img_size[1])
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res['content-type'], 'image/jpeg')
    ...
    def test11BoreholeImageLarge(self):
        ...
        Image.objects.create(borehole=self.bh1, depth_from=500, depth_to=10500, geophysical_depth=0, imagedata=self.imgsize.getvalue())
        self.req.method = 'GET'
        t0 = time.time()
        img_size = (100, 100)

        res = views.borehole_image(self.req, self.bh1.id, 0, 1000, img_size[0], img_size[1])
    ...
    def test12Archiver(self):
        ...
        self.assertEqual(int(float(archiver.progress) / float(archiver.max) * 100), 100)
    ...
    def test14incompleteArchiveProcessing(self):
        ...
        with zf.ZipFile(archIncomplete, 'w') as temp:
            for i in [890, 892, 897, 903, 904, 905, 906, 908, 909, 910, 1004, 1100, 1299]:
                temp.writestr(str(i) + '.jpg', self.imgw.getvalue())
        ...
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=1000'])), 6)
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=10000'])), 5)
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=100000'])), 2)
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=%d' % (settings.MAX_BOREHOLE_HEIGHT 
                                                                                      * settings.MEASUREMENT_IMAGE_HEIGHT_CM)])),
                         1)
    ...
    def test15calculateImgHeight(self):
        ...
        self.assertLessEqual(views.calculateImgHeight(settings.MAX_BOREHOLE_HEIGHT), settings.MAX_BOREHOLE_HEIGHT)
    
    def test16calculateHelpersNumber(self):
        archiver = images.archiver.Archiver()
        
        self.assertEqual(archiver.calculateHelpersNumber([890, 892, 897, 903, 904, 905, 906, 908, 909, 910, 1004, 1100, 1299]),
                         14)
        
        self.assertEqual(archiver.calculateHelpersNumber([i for i in range(settings.MAX_BOREHOLE_HEIGHT)]),
                         settings.MAX_BOREHOLE_HEIGHT // 10 + settings.MAX_BOREHOLE_HEIGHT // 100 + 
                         settings.MAX_BOREHOLE_HEIGHT // 1000 + 1)
    ...
    def test17doRegenerate(self):
        ...
        Image.objects.create(borehole = self.bh1, depth_from = 135000, depth_to = 135100, geophysical_depth = 135000,
        ...
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=1000'])), 7)
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=10000'])), 6)
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=100000'])), 2)
        self.assertEqual(len(Image.objects.extra(where=['depth_to - depth_from=%d' % (settings.MAX_BOREHOLE_HEIGHT 
                                                                                      * settings.MEASUREMENT_IMAGE_HEIGHT_CM)])),
                         1)
    ...
    def test18regenerationRequest(self):
        ...
        self.req.read = lambda : json.dumps({'borehole_id' : self.bh1.id}).encode()
    ...
    def test20getArchiveProgress(self):
        ...
        archiver.max = 0
        self.assertDictEqual(json.loads(views.getArchiveProgress(self.req).content.decode('utf-8')), {'status' : ProgressStatus.DUPLICATE_ERR,
                                                                             'progress' : 0})

        archiver.status = ProgressStatus.BAD_SIZE_ERR
        archiver.progress = 1
        archiver.max = 1
        self.assertDictEqual(json.loads(views.getArchiveProgress(self.req).content.decode('utf-8')), {'status' : ProgressStatus.BAD_SIZE_ERR,
                                                                             'progress' : 100})
        ...
        archiver.progress = 0
        archiver.max = -1
        self.assertDictEqual(json.loads(views.getArchiveProgress(self.req).content.decode('utf-8')), {'status' : ProgressStatus.FINISHED,
                                                                             'progress' : 0})
