...
class UserInterruptException(Exception): pass

class ProgressStatus:
    OK = 'Ok'
    DUPLICATE_ERR = 'Archive has duplicated file names'
    BAD_SIZE_ERR = "Size {} {}".format(settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX)
...
class Archiver(object):
    ...
    def startRegeneration(self, request, borehole):
        self._reset()
        self.worker = threading.Thread(target = self.doRegenerate, args=(request, borehole,))
        self.worker.start()
    ...
    def isBusy(self):
        return (self.worker != None and self.worker.isAlive())
    ...
    def generateHelperPhotos(self, base_photos, borehole):
        ...
        out10 = Image.new("RGBA", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX * 10))
        ...
        for i, photo in enumerate(base_photos):
            ...
            if photodata.size != (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX):
                photodata = photodata.resize((settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX), resample = Image.ANTIALIAS)
            out10.paste(photodata, (0, imagePosHeight(photo.depth_from - first10)))
                                        
            counter = i + 1
            for index, j in enumerate(helper_tab):
                if counter >= img_num or base_photos[counter].depth_from // j * j >= locals()['max%d' %j]:
                    t = BytesIO()
                    locals()['out%d' % j].resize((settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX), resample = Image.ANTIALIAS).save(t, format = 'JPEG')
                    t.seek(0)
                    if index < len(helper_tab) - 1:
                        locals()['out%d' % helper_tab[index + 1]].paste(Image.open(BytesIO(t.read())), (0, int(imagePosHeight(locals()['first%d' % j] - locals()['first%d' % helper_tab[index+1]]) / j)))
                        t.seek(0)
                    add_image(t.read(), borehole = borehole, depth_from = locals()['first%d' % j], depth_to = locals()['max%d' % j], geophysical_depth = locals()['first%d' % j])
                    self.progress += 1
                    
                    if counter < img_num:
                        if j == 10:
                            first10 = base_photos[counter].depth_from // (100 * j) * (100 * j)
                            max10 = locals()['first%d' % j] + int(settings.MEASUREMENT_IMAGE_HEIGHT_CM) * j
                            out10 = Image.new("RGBA", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX * 10))
                        elif j == 100:
                            first100 = base_photos[counter].depth_from // (100 * j) * (100 * j)
                            max100 = locals()['first%d' % j] + int(settings.MEASUREMENT_IMAGE_HEIGHT_CM) * j
                            out100 = Image.new("RGBA", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX * 10))
                        elif j == 1000:
                            first1000 = base_photos[counter].depth_from // (100 * j) * (100 * j)
                            max1000 = locals()['first%d' % j] + int(settings.MEASUREMENT_IMAGE_HEIGHT_CM) * j
                            out1000 = Image.new("RGBA", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX * 10))
                else:
                    break

        added_photos = models.Image.objects.filter(borehole = borehole, meaning = None).extra(where=['depth_to - depth_from=100000']).order_by('depth_from')

        if not len(added_photos):
            return
        
        outfull = Image.new("RGBA", (settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX * 
                                     (settings.MAX_BOREHOLE_HEIGHT // 1000)))
        
        for img in added_photos:
            outfull.paste(Image.open(BytesIO(img.imagedata)), (0, int(imagePosHeight(img.depth_from) / 2 / (settings.MAX_BOREHOLE_HEIGHT // 10))))
                 
        t = BytesIO()
        outfull.resize((settings.MEASUREMENT_IMAGE_WIDTH_PX, settings.MEASUREMENT_IMAGE_HEIGHT_PX), resample=Image.ANTIALIAS).save(t, format='JPEG')
        t.seek(0)
        add_image(t.read(), borehole = borehole, depth_from = 0, depth_to = settings.MAX_BOREHOLE_HEIGHT * settings.MEASUREMENT_IMAGE_HEIGHT_CM, 
                  geophysical_depth = 0)
        self.progress += 1
    ...
    def doRegenerate(self, request, borehole):
        try:
            ...
            self.max = temp = len(imgs.extra(where=['depth_to - depth_from>100']))
            imgs = imgs.order_by('depth_from') 
            self.max += self.calculateHelpersNumber([img.depth_from / settings.MEASUREMENT_IMAGE_HEIGHT_CM for img in imgs.extra(where=['depth_to - depth_from=100'])])

            with transaction.atomic():
                imgs.extra(where=['depth_to - depth_from>100']).delete()
                self.progress += temp
                self.generateHelperPhotos(imgs, borehole)
                logger.info("User %s regenerated photos" % request.user.username)
 ...
 #           data = BytesIO(file)
        ...
        with zf.ZipFile(data, 'r') as arch:
            ...
            try:
                ...
                logger.error('upload_archive exception: integrity error, ' + str(e) )
