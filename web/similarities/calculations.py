...
def calculatePeriods(periodslist):
    ...
    for p in periodslist:
        if not len(periods[p.dictionary_id]
                   ) or periods[p.dictionary_id][-1]['to'] < p.depth_from:
            periods[p.dictionary_id].append(
                {'from': p.depth_from, 'to': p.depth_to})
        else:
...
def calculateMetrics(vals, periods, variances):
    ...
    for p in periods:
            ...
            if lastmeaning != v.meaning_id:
                if lastmeaning and num:
                    if variances[lastmeaning]:
                        ret[(lastmeaning, p)] = abs(
                            ret[(lastmeaning, p)] / num) / math.sqrt(variances[lastmeaning])
                    else:
                        ret[(lastmeaning, p)] = 0
        ...
        if lastmeaning and num:
            if variances[lastmeaning]:
                ret[(lastmeaning, p)] = abs(ret[(lastmeaning, p)] / \
                     num) / math.sqrt(variances[lastmeaning])
            else:
                ret[(lastmeaning, p)] = 0
...
def comparision(borehole_id, vals, strats):
    ...
    localmetrics = calculateMetrics(
        data[borehole_id], calculatePeriods(
            strats[borehole_id]), variances)
    ...
    for d in data:
        ...
        ret[data[d][0].bh] = math.sqrt(
            dst) if metricsfound else 'NO_COMMON_METRICS'

    for b in Borehole.objects.exclude(id=borehole_id):
        if b.name not in ret:
...
def compareTo(borehole_id, stratigraphy_level, epochs, meanings=None):
    ...
    vals = RealMeasurement.objects.extra(
        {'bh': 'SELECT name FROM boreholes_borehole WHERE id=measurements_measurement.borehole_id'}).all()
    if meanings:
        meanings = [int(m) for m in meanings]
        vals = vals.filter(
            meaning__in=MeaningValue.objects.filter(
                id__in=meanings).exclude(
                unit__in=[
                    'DICT', 'PICT']))
    else:
        meanings = [
            m.id for m in MeaningValue.objects.exclude(
                unit__in=[
                    'DICT', 'PICT'])]
    ...
    for d in DictionaryMeasurement.objects.extra(
            {
                'bh': 'SELECT name FROM boreholes_borehole WHERE id=measurements_measurement.borehole_id'}). filter(
        meaning=dict,
        dictionary__in=dictvalues).order_by(
                    'dictionary',
            'depth_from'):
