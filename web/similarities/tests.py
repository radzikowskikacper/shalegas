...
class periodtype(namedtuple('period', ['dictionary_id', 'depth_from', 'depth_to', 'bh', 'borehole_id'])):
    def __new__(cls, dictionary_id = -1, depth_from = -1, depth_to = -1, bh = None, borehole_id = -1):
        return super(periodtype, cls).__new__(cls, dictionary_id, depth_from, depth_to, bh, borehole_id)
    
class datatype(namedtuple('datatype', ['value', 'depth_from', 'depth_to', 'meaning_id', 'bh', 'borehole_id'])):
    def __new__(cls, value = -1, depth_from = -1, meaning_id = -1, depth_to = -1, borehole_id = -1, bh = None):
        return super(datatype, cls).__new__(cls, value, depth_from, meaning_id, depth_to, borehole_id, bh)
...
class SimilaritiesViewsTestCase(django.test.TestCase):
    ...
    def createTestData(self):    
        ...
        for i in stratigraphy_list:
            m = MeaningDict.objects.create(id = i, name = self.test_meaning_name + str(i), unit = 'DICT', section = sects[i % self.tests_num])
            for j in range(1, self.tests_num + 1):
                temp += 1
                MeaningDictValue.objects.create(id = temp, value = self.test_meaning_name + '_value' + str(j) + '_' + str(i), dict_id = m)
        ...
        for i in range(self.tests_num):
            for j in range(self.tests_large_num):
                RealMeasurement.objects.create(value = self.test_value + float(i * j), meaning = meanings[j%self.tests_num], 
                                                           borehole = bhs[i], depth_from = self.test_depth + j* 10 * i, 
    ...
    def setUp(self):
        ...
        self.request.user = self.user = User.objects.create_user(self.test_username, '', self.test_password)
    
    def test02compareTo(self):
        self.createTestData()
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[1], [i for i in range(self.tests_num, self.tests_num*2)]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        ...
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[0], [i for i in range(1, self.tests_num + 1)]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})

        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 0, depth_to = 47900, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[0]),
                                             dictionary = MeaningDictValue.objects.get(id = 1))
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[1], [i for i in range(self.tests_num, self.tests_num*2)]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        
        bh = Borehole.objects.create(name = self.test_borehole_name + '2', latitude = self.test_lat, longitude = self.test_lon)
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[1], [i for i in range(self.tests_num, self.tests_num*2)]), {self.test_borehole_name + '2' : 'NO_MEASUREMENTS',
                                                                        self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        bh.delete()
        
        localdata = RealMeasurement.objects.filter(borehole_id = 1).order_by('meaning', 'depth_from')
        data = RealMeasurement.objects.filter(borehole_id = 2).order_by('meaning', 'depth_from')
        temp = RealMeasurement.objects.all().order_by('meaning', 'depth_from')        
        avgs = calculateAVGs(temp)
        varc = calculateVariances(temp, avgs)
        localperiods = calculatePeriods(DictionaryMeasurement.objects.filter(borehole_id = 1).order_by('dictionary', 'depth_from'))
        periods = calculatePeriods(DictionaryMeasurement.objects.filter(borehole_id = 2).order_by('dictionary', 'depth_from'))
        ...
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[0], [i for i in range(1, self.tests_num + 1)]), {self.test_borehole_name + '1' : math.sqrt(dst)})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[0], [i for i in range(2, self.tests_num + 1)]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        ...
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[0], [i for i in range(1, self.tests_num + 1)], [1]), {self.test_borehole_name + '1' : math.sqrt(dst)})
    ...
    def test021compareTo(self):
        ...
        DictionaryMeasurement.objects.create(borehole_id = 1, depth_from = 0, depth_to = 500000, geophysical_depth = 0,
                                             ...
                                             dictionary = MeaningDictValue.objects.get(id = 1))
        DictionaryMeasurement.objects.create(borehole_id = 1, depth_from = 0, depth_to = 500000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[1]),
                                             dictionary = MeaningDictValue.objects.get(id = 3))
        DictionaryMeasurement.objects.create(borehole_id = 1, depth_from = 0, depth_to = 500000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[2]),
                                             dictionary = MeaningDictValue.objects.get(id = 5))

        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 0, depth_to = 250000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[0]),
                                             dictionary = MeaningDictValue.objects.get(id = 1))
        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 250000, depth_to = 500000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[0]),
                                             dictionary = MeaningDictValue.objects.get(id = 1))
        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 0, depth_to = 10000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[1]),
                                             dictionary = MeaningDictValue.objects.get(id = 3))
        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 10000, depth_to = 500000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[1]),
                                             dictionary = MeaningDictValue.objects.get(id = 4))
        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 0, depth_to = 9000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[2]),
                                             dictionary = MeaningDictValue.objects.get(id = 5))
        DictionaryMeasurement.objects.create(borehole_id = 2, depth_from = 9000, depth_to = 500000, geophysical_depth = 0,
                                             meaning = MeaningDict.objects.get(id = stratigraphy_list[2]),
                                             dictionary = MeaningDictValue.objects.get(id = 6))
        ...
        dst = 0
        for (m, i) in set(localmetrics.keys()).intersection(set(metrics.keys())):
            dst += (metrics[(m, i)] - localmetrics[(m, i)])**2

        self.assertDictEqual(views.compareTo(1, stratigraphy_list[0], [i for i in range(1, self.tests_num + 1)]), {self.test_borehole_name + '1' : math.sqrt(dst)})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[1], [3]), {self.test_borehole_name + '1' : 0})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[1], [4]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[1], [3, 4]), {self.test_borehole_name + '1' : 0})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[2], [5]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[2], [5, 6]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
        self.assertDictEqual(views.compareTo(1, stratigraphy_list[2], [6]), {self.test_borehole_name + '1' : 'NO_COMMON_METRICS'})
    ...
    def test03calculateAVGs(self):
        data = [datatype(self.test_value + i, i, i+1, 1) for i in range(25000, 475000)]
        self.assertEqual(calculateAVGs(data), {1 : sum(item.value for item in data) / len(data)})
        
        data = [datatype(self.test_value * i, i, i+1, j) for j in range(1, 4) for i in range(25000, 475000)]
        self.assertDictEqual(calculateAVGs(data), {1 : sum(item.value for item in data[:450000]) / len(data[:450000]),
    ...
    def test05calculateVariances(self):
        ...
        self.assertEqual(calculateVariances(data, {1 : avg}), {1 : sum([abs(item.value - avg)**2 for item in data]) / (len(data)-1)})
        
        data = [datatype(self.test_value * i, i, i + 1, j) for j in range(1, 4) for i in range(25000, 475000)]
        avgs = {1 : sum(item.value for item in data[:450000]) / len(data[:450000]),
                2 : sum(item.value for item in data[450000:900000]) / len(data[450000:900000]),
                3 : sum(item.value for item in data[900000:1350000]) / len(data[900000:1350000])}
        self.assertDictEqual(calculateVariances(data, avgs), {1 : sum(abs(item.value - avgs[1])**2 for item in data[:450000]) / (len(data[:450000])-1),
    ...
    def test06getSimilarities(self):
        ...
        self.assertEqual(len(json.loads(views.similarities(self.request, 1).content.decode('utf-8'))), 1)
                    
        self.assertRaises(Borehole.DoesNotExist, views.similarities, self.request, 3)   
           
    def test07calculatePeriods(self):
        periods = [periodtype(1, self.test_depth + i * 100000, self.test_depth + i * 100000 + 50000) for i in range(self.tests_num)] + \
                   [periodtype(2, self.test_depth + i * 100000, self.test_depth + i * 100000 + 50000) for i in range(self.tests_num, self.tests_num*2)] + \
                   [periodtype(3, self.test_depth + i * 100000, self.test_depth + i * 100000 + 50000) for i in range(self.tests_num*2, self.tests_num*3)]
                   
        self.assertDictEqual(calculatePeriods(periods), 
                             {1: [{'from' : self.test_depth, 'to' : self.test_depth + 50000}, 
                                  {'from' : self.test_depth + 100000, 'to' : self.test_depth + 100000 + 50000}],
                              2 : [{'from' : self.test_depth + self.tests_num * 100000, 'to' : self.test_depth + self.tests_num * 100000 + 50000},
                                   {'from' : self.test_depth + (self.tests_num + 1) * 100000, 'to' : self.test_depth + (self.tests_num + 1) * 100000 + 50000}],
                              3 : [{'from' : self.test_depth + 2 * self.tests_num * 100000, 'to' : self.test_depth + 2 * self.tests_num * 100000 + 50000},
                                   {'from' : self.test_depth + (2 * self.tests_num + 1) * 100000, 'to' : self.test_depth + (2 * self.tests_num + 1) * 100000 + 50000}]})

        
        periods = [periodtype(3, self.test_depth + i * 100000, self.test_depth + i * 100000 + 100000) for i in range(self.tests_num)] + \
                   [periodtype(4, self.test_depth + i * 100000, self.test_depth + i * 100000 + 150000) for i in range(self.tests_num, self.tests_num*2)]

        self.assertDictEqual(calculatePeriods(periods), 
                             {3 : [{'from' : self.test_depth, 'to' : self.test_depth + (self.tests_num - 1) * 100000 + 100000}],
                              4 : [{'from' : self.test_depth + self.tests_num * 100000, 'to' : self.test_depth + (2 * self.tests_num - 1) * 100000 + 150000}]})
    ...
    def test08calculateMetrics(self):
        ...
        avgs = {1 : 30, 2 : 45, 3 : 56}
        vals = [datatype(self.test_value + i, self.test_depth + i * 10000, self.test_depth + i * 10000 + 1, 1) for i in range(self.tests_num)] + \
                [datatype(self.test_value + i * 2, self.test_depth + (i + 1) * 100000, self.test_depth + (i + 1) * 100000 + 1, 2) 
                 for i in range(self.tests_num)] + \
                [datatype(self.test_value + i * 3, self.test_depth + (i + 2) * 10000, self.test_depth + (i + 2) * 10000 + 1, 3) 
                 for i in range(self.tests_num)]
        ...
        vals = [datatype(self.test_value + i, self.test_depth + i * 10000, self.test_depth + i * 10000 + 1, 1) for i in range(20)] + \
                [datatype(self.test_value + i * 2, self.test_depth + (i + 1) * 10000, self.test_depth + (i + 1) * 10000 + 1, 2) 
                 for i in range(20)] + \
                [datatype(self.test_value + i * 3, self.test_depth + (i + 2) * 10000, self.test_depth + (i + 2) * 10000 + 1, 3) 
                 for i in range(20)]
