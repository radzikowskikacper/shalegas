...
class DataModelTestCase(django.test.TestCase):
    ...
    def createTestMeasurements(self):
        ...
        for i in range(1, self.tests_num + 1):
            dictvals.append(MeaningDictValue.objects.create(
                dict_id=meanings[self.tests_num + i - 1], value=self.test_dict_value + str(i)))
        ...
        for i in range(self.tests_num):
            for j in range(self.tests_num):
                RealMeasurement.objects.create(
                    ...
                    j *
                    100)
                DictionaryMeasurement.objects.create(dictionary=dictvals[j],
    ...
    def test01allBoreholesData(self):
        ...
        self.assertListEqual(res['meanings'],
                             [{'name': self.test_meaning_name + str(i),
                               'unit': self.test_unit + str(i)} for i in range(1,
                                                                               3)] + [{'name': self.test_meaning_name + str(i),
                                                                                       'unit': 'DICT'} for i in range(3,
                                                                                                                      5)])
        self.assertListEqual(res['boreholes'], [self.test_borehole_name + str(i)
