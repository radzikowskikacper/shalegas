...
def data(request):
    if request.method == 'GET':
        with transaction.atomic():
            ...
            curdepth = None
            ret = {'boreholes' : [], 'meanings' : [m.id for m in meanings.values()]}
            data = []
            row = None
                
            for d in sorted(list(vals) + list(dicts), key = lambda x: (x.depth_from, x.borehole_id, x.meaning_id)):
                ...
                if not d.borehole_id in ret['boreholes']:
                    ret['boreholes'].append(d.borehole_id)
                    
                row += ['' for i in range(ret['boreholes'].index(d.borehole_id) * len(ret['meanings']) - len(row) + 1)]
          
                if len(ret['meanings']) > 1:
                    row += ['' for i in range(ret['meanings'].index(d.meaning_id) - (len(row) - 1) % len(ret['meanings']))]
            ...
            ret['meanings'] = [{'name' : m.name, 'unit' : m.unit} for m in meanings.values()]
            ret['data'] = data
